	...
	float screenDepth, float screenNear)
{
	...
	D3DXMatrixPerspectiveFovLH(&pProjectionMatrix, fieldOfView, screenAspect, screenNear, screenDepth);

	// Initialize the world matrix to the identity matrix.
	D3DXMatrixIdentity(&pWorldMatrix);

	// Create an orthographic projection matrix for 2D rendering.
	D3DXMatrixOrthoLH(&pOrthoMatrix, (float)screenWidth, (float)screenHeight, screenNear, screenDepth);
...
{
	pProjectionMatrix = pProjectionMatrix;
	return;
}
